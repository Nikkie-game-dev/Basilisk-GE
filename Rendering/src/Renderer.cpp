...
namespace basilisk::rendering
{
    void Renderer::InitGLFW()
    {
        utils::Log::Get()->info("Loading GLFW");
        if (!glfwInit())
        {
            const char* description;
            int errorCode = glfwGetError(&description);

            utils::Log::Get()->error("GLFW failed to initialize with error code {}.\n Error description: {}", errorCode, std::string(description));
            abort();
        }
        utils::Log::Get()->info("GLFW loaded");
    }
    ...
    void Renderer::InitGL() const
    {
        ...
        utils::Log::Get()->info("Loading GLEW");
        if (const unsigned int errorCode = glewInit(); errorCode != GLEW_OK)
        {
            utils::Log::Get()->error("GLEW failed to initialize with error code {}.\n Error description: {}", errorCode,
        ...
        utils::Log::Get()->info("GLEW loaded");
    }
    ...
    void Renderer::GenerateVBs(Buffers& buffers, bool isTextured)
    {
        ...
        const int stride = posSize + utils::Color::ColorParamsAmount + textureSize;
        ...
        //Color
        SetAttribPointer(1, utils::Color::ColorParamsAmount, stride, posSize);

        if (isTextured)
        {
            SetAttribPointer(2, textureSize, stride, posSize + utils::Color::ColorParamsAmount);
        }
    ...
    void Renderer::SetWindowRef(rendering::Window& window)
    {
        this->Window = &window;
    }
