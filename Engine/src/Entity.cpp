...
namespace basilisk
{
    ...
    void Entity::SetScaling(const vec3& scaling)
    {
        this->Scaling = scaling;
        this->ScaleMatrix = scale(mat4(1.0f), this->Scaling);
        UpdateModelMatrix();
    }
    ...
    vec3 Entity::GetRotation(const bool isRads) const
    {
        if (!isRads)
        {
    ...
    const vec3* Entity::GetPositionPtr() const
    {
        return &this->Position;
    }
    ...
    void Entity::UpdateRotationMatrix()
    {
        ...
        this->RotationMatrix = rotate(this->RotationMatrix, this->Rotation.x, vec3(1.0f, 0.0f, 0.0f));

        this->RotationMatrix = rotate(this->RotationMatrix, this->Rotation.y, vec3(0.0f, 1.0f, 0.0f));

        this->RotationMatrix = rotate(this->RotationMatrix, this->Rotation.z, vec3(0.0f, 0.0f, 1.0f));
    }

    void Entity::UpdateTranslateMatrix()
    {
        this->TranslateMatrix = translate(mat4(1.0f), this->Position);
    }
