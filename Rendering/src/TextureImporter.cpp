...
namespace basilisk::rendering
{
    ...
    {
        utils::Log::Get()->info("Loading Image at {}", imageDir);

        if (!std::filesystem::exists(imageDir))
        {
            utils::Log::Get()->error("Image at {} not found", imageDir);
            return nullptr;
        }
        ...
        if (!data)
        {
            utils::Log::Get()->error("Could not load data at {}", imageDir);
            return nullptr;        
        }
    ...
    unsigned int TextureImporter::GenTexture(unsigned char* data,
                                             ...
                                             const FitMode& fit)
    {
        utils::Log::Get()->info("Generating texture");
        ...
        utils::Log::Get()->info("Texture Id: {}", texture);
